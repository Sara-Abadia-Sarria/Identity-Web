...
namespace Identity.Web
{
    public class EmailService : IIdentityMessageService
    {
        public async Task SendAsync(IdentityMessage message)
        {
            ...
            await SendEmailConfirmationTokenAsync(message);
        }

        // Use NuGet to install SendGrid (Basic C# client lib) 
        private async Task SendEmailConfirmationTokenAsync(IdentityMessage message)
        {
            var client = new SendGridClient(Helpers.Utils.stDecrypt(ConfigurationManager.AppSettings["SendGridKey"]));

            var myMessage = new SendGridMessage();
            myMessage.AddTo(message.Destination);
            myMessage.From = new EmailAddress("santafedavid01@gmail.com", "David Santafe Zorrilla");
